...
All agents use `Codex-opus-4-8` with `thinking: {type: "adaptive"}` and streaming.
...
- **Streaming by default**: All Codex API calls use streaming (`.stream()`) and `.get_final_message()` to avoid timeouts on large documents.
- **Adaptive thinking**: All agents use `thinking: {type: "adaptive"}` for accurate procurement domain reasoning.
- **Tool use for document reading**: Document parsing is wrapped as Codex tools so agents can request re-reads of specific sections.
